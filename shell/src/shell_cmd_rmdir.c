...
static Error rm_path (const char *argv0, const char *path, bool verbose)
  ...
  if (fsutil_is_directory (path) == false)
    {
    compat_printf_stderr ("Not deleting non-directory '%s'\n", path);
    ret = EINVAL;
    }
  else
    {
    Error ret = sys_rmdir (path);
    if (ret)
...
/*=========================================================================
  shell_cmd_rmdir
=========================================================================*/
Error shell_cmd_rmdir (int argc, char **argv)
  {
  int opt;
  Error ret = 0;
  optind = 0;
